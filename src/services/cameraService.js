...
class CameraService {
  ...
  validateDetectionData(data) {
    ...
    let isChilean = /^[A-Z]{4}\d{2}$/.test(normalized) || /^[A-Z]{2}\d{4}$/.test(normalized);
    if (!isChilean && normalized.length === 6) {
      const mapDigit = (ch) => {
      ...
      isChilean = /^[A-Z]{4}\d{2}$/.test(asModern) || /^[A-Z]{2}\d{4}$/.test(asOld);
      if (isChilean) {
        data.license_plate = /^[A-Z]{4}\d{2}$/.test(asModern) ? asModern : asOld;
      }
    }
    if (!isChilean) {
